...
uses
  ...
  StdCtrls, ExtCtrls, Menus, MCFuncProc, StrUtils, types, ComCtrls, LResources,
...
resourcestring
  ...
  rsCEAFilter = 'MyCheat Assembly (*.CEA)|*.CEA|All Files ( *.* )|*.*';
...
{  assemblescreen.SelStart:=0;
  ...
  try
    if reg.OpenKey('\Software\MyCheat\Auto Assembler\',false) then
    begin
  ...
  begin
    try
      ...
      begin
        ...
        reg:=tregistry.create;
        try
          if reg.OpenKey('\Software\MyCheat\Auto Assembler\',true) then
          begin
...
begin
  {$ifndef standalonetrainerwithassembler}
    ...
    begin
      ...
      // same as menu option "MyCheat framework code", make sure we
