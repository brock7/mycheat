    ...
    try
      ...
      begin
        ...
        begin
          ...
          if reg.ValueExists('Show MyCheat Hotkey') then
            reg.ReadBinaryData('Show MyCheat Hotkey',temphotkeylist[0][0],10) else  mainform.label7.Caption:='';
      ...
      if Reg.OpenKey('\Software\MyCheat\Tools',false) then
      begin
      ...
      if Reg.OpenKey('\Software\MyCheat\Plugins'{$ifdef cpu64}+'64'{$else}+'32'{$endif},false) then
      begin
        names:=TStringList.create;
        try
          reg.GetValueNames(names);
