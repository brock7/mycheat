...
begin
  ...
  if t=lowercase(eng_vtByte) then
    result:=eng_vtByte else
  if t=lowercase(eng_vtWord) then
    result:=eng_vtWord else
  if t=lowercase(eng_vtDword) then
    result:=eng_vtDword else
  if t=lowercase(eng_vtQword) then
    result:=eng_vtQword else
  if t=lowercase(eng_vtByte) then
    result:=eng_vtByte else
  if t=lowercase(eng_vtSingle) then
    result:=eng_vtSingle else
  if t=lowercase(eng_vtDouble) then
    result:=eng_vtDouble else
  if t=lowercase(eng_vtString) then
    result:=eng_vtString else
  if t=lowercase(eng_vtUnicodeString) then
    result:=eng_vtUnicodeString else
  if t=lowercase(eng_vtPointer) then
    result:=eng_vtPointer else
  if t=lowercase(eng_vtAutoAssembler) then
    result:=eng_vtAutoAssembler else
  if t=lowercase(eng_vtCustom) then
    result:=eng_vtCustom;
