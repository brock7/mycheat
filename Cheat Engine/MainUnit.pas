...
begin
  ...
  try
    if reg.OpenKey('\Software\MyCheat\CustomTypes\', False) then
    begin
      CustomTypes := TStringList.Create;
      reg.GetKeyNames(CustomTypes);
      for i := 0 to CustomTypes.Count - 1 do
      begin
        if reg.OpenKey('\Software\MyCheat\CustomTypes\' + CustomTypes[i], False) then
        begin
          try
            islua := False;
...
begin
  ...
  begin
    ...
    begin
      reg := tregistry.Create;
      reg.DeleteKey('\Software\MyCheat\CustomTypes\' + ct.Name);
      ct.remove;
      RefreshCustomTypes;
    end;
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        //delete the old one
        reg := Tregistry.Create;
        reg.DeleteKey('\Software\MyCheat\CustomTypes\' + oldname);
        reg.Free;
      end;
    end;
    ...
    reg := Tregistry.Create;
    if Reg.OpenKey('\Software\MyCheat\CustomTypes\' + ct.Name, True) then
    begin
...
begin
  if messagedlg(rsDoYouWantToGoToTheCheatEngineWebsite, mtConfirmation,
    [mbYes, mbNo], 0) = mrYes then
    ShellExecute(0, PChar('open'), PChar('http://www.mycheat.org/?referredby=CE63'),
...
begin
  ...
  try
    ...
    if not Reg.OpenKey('\Software\MyCheat', False) then
    begin
      if Reg.OpenKey('\Software\MyCheat', True) then
      begin
